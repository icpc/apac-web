...
export default function SponsorsGrid({ year }: SponsorsGridProps) {
            ...
            .then((data: SponsorsData) => {
                ...
                const allProcessedLevels = data.map(levelEntry => {
                    ...
                    levelSponsorsEntries.forEach(sponsorEntry => {
                        ...
                        if (currentImageLine.length > 0 && (currentImageLineSizeSum + sponsor.size) > MAX_LENGTH) {
    ...
    if (displayableLevels.length > 0) {
        ...
        displayableLevels.forEach(levelInfo => {
            ...
            if (currentRow.length > 0 && (currentRowCombinedSize + levelInfo.levelTotalSize) > MAX_LENGTH) {
    ...
    console.log('Rows of Cards:', rowsOfCards);

    return (
        <div className="w-full py-4">
            <div className="w-full mx-auto">
                {rowsOfCards.map((cardRow, rowIndex) => (
                    <div key={`card-row-${rowIndex}`} className="flex flex-wrap justify-start gap-4 mb-4"> {/* justify-start or justify-center? */} 
                        {cardRow.map(levelDetails => {
                            const cardBaseClasses = `min-w-0 mx-2`;
                                    ...
                                    <CardContent className="h-full p-6 flex flex-col">
                                        ...
                                        <div className="flex-grow flex flex-col justify-center items-center">
                                            <div
                                                ...
                                                style={cardRow.length === 1 ? { width: `${Math.min(100, (levelDetails.levelTotalSize / MAX_LENGTH) * 100)}%` } : {}}
                                            >
